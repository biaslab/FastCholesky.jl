...
benchmark: ## Run benchmarks
	julia -e 'ENV["USE_DEV"]="$(dev)"; import Pkg; Pkg.activate("benchmarks/speed"); using PkgBenchmark, FastCholesky, Dates; results = benchmarkpkg("FastCholesky"; script="benchmarks/speed/benchmarks.jl"); export_markdown("benchmarks/speed/exports/benchmark-"*Dates.format(Dates.now(), "yyyy-mm-ddTHH-MM")*".md", results);'